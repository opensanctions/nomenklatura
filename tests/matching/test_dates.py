...
def test_dob_matches():
    ...
    assert dob_day_disjoint(left, right, config).score == 0.0
    assert dob_year_disjoint(left, right, config).score == 0.0
    ...
    assert dob_day_disjoint(left, right, config).score == 1.0
    assert dob_year_disjoint(left, right, config).score == 1.0
    none = e("Person", name="Harry")
    assert dob_matches(left, none) == 0.0
    assert dob_year_matches(left, none) == 0.0
    assert dob_day_disjoint(left, none, config).score == 0.0
    assert dob_year_disjoint(left, none, config).score == 0.0
