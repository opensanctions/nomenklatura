...
from nomenklatura.matching import compare_v2_scored, explain_v2_matcher
...
def test_explain_matcher():
    explanation = explain_v2_matcher()
...
def test_compare_entities():
    ...
    res_match = compare_v2_scored(cand, match)
    res_mismatch = compare_v2_scored(cand, mismatch)
...
def test_compare_features():
    cand = Entity.from_dict(model, candidate)
    match = Entity.from_dict(model, putin)
    ref_match = compare_v2_scored(cand, match)
    ref_score = ref_match["score"]

    no_bday = match.clone()
    no_bday.pop("birthDate")
    bday_match = compare_v2_scored(cand, no_bday)
    assert ref_score > bday_match["score"]

    bela = match.clone()
    bela.set("nationality", "by")
    bela_match = compare_v2_scored(cand, bela)
