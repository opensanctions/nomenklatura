...
from nomenklatura.matching import RandomForestV1
...
def test_compare_entities():
    ...
    res_match = RandomForestV1.compare(cand, match)
    res_mismatch = RandomForestV1.compare(cand, mismatch)
...
def test_compare_features():
    cand = Entity.from_dict(model, candidate)
    match = Entity.from_dict(model, putin)
    ref_match = RandomForestV1.compare(cand, match)
    ref_score = ref_match.score

    no_bday = match.clone()
    no_bday.pop("birthDate")
    bday_match = RandomForestV1.compare(cand, no_bday)
