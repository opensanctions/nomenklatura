...
[Wikidata](https://www.wikidata.org/) is a useful source of structured data on politicians, companies, and other entities of interest. This module is the low-level client used by the Wikidata enricher in `nomenklatura.enrich` and by crawlers that turn Wikidata items into FollowTheMoney entities. It handles the parts that are error-prone to reimplement: request throttling and retries, response caching via a `nomenklatura.cache.Cache`, and picking a display label from the many languages an item may carry.
