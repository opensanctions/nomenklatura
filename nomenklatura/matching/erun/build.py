...
class SnapshotGroup:
    ...
    def add(self, label: str, row_number: int) -> None:
...
def snapshot_summary(
    ...
    """Summarize deduplication and contradictions for the build report."""

    contradictory = [group for group in groups.values() if group.contradictory]
    clean = [group for group in groups.values() if not group.contradictory]
    return {
        "format_version": 1,
        "scan": stats.to_dict(),
        "snapshots": {
            ...
            "duplicate_rows_beyond_first": stats.eligible_rows - len(groups),
            "quarantined_rows": sum(group.count for group in contradictory),
        },
    }


def assign_partitions(
    groups: Iterable[SnapshotGroup], test_size: float, seed: int
) -> dict[str, str]:
    """Keep snapshots together while stratifying the split by schema and label."""
    ...
    for stratum in sorted(strata):
        members = sorted(strata[stratum], key=lambda group: group.digest)
        rng.shuffle(members)
        if len(members) < 2:
            test_count = 0
        else:
            test_count = round(len(members) * test_size)
            test_count = min(len(members) - 1, max(1, test_count))
        for index, group in enumerate(members):
            partitions[group.digest] = "test" if index < test_count else "train"
    return partitions
...
def prepare_manifest(
    ...
    groups, stats = collect_snapshot_groups(pairs_file)
    clean = [group for group in groups.values() if not group.contradictory]
    contradictory = [group for group in groups.values() if group.contradictory]
    partitions = assign_partitions(clean, test_size, seed)
    ...
    manifest_records = (
        {
            ...
            "partition": partitions[group.digest],
    ...
    quarantine_records = (
        ...
        for group in sorted(contradictory, key=lambda group: group.digest)
    )
    ...
    partition_groups = Counter(partitions.values())
    partition_rows: Counter[str] = Counter()
    development_strata: Counter[str] = Counter()
    for group in clean:
        partition_rows[partitions[group.digest]] += group.count
...
def iter_selected_pairs(
    ...
    stats = ScanStats()
    for row_number, pair in iter_eligible_pairs(pairs_file, stats):
